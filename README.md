...
[*] Due to the fact that in the commercetools there can be a lot of possible prices per one product, but in Voucherify there is only one, it is needed to choose proper price. You can find proper values by searching in `*any product* > Variants > *any variant* > Prices` for row which have proper price scope for your project. Depends on sitiuation you may need to use all of four variables or less, but at least the `COMMERCE_TOOLS_PRODUCTS_CURRENCY` variable is required. In case when you does not choose right values, then some functionallities won't work properly (e.g. adding free item with coupon). [Here](https://docs.commercetools.com/api/projects/products#price-selection) you can read more about commercetools mechanism called "Price Selection" which we used. 
