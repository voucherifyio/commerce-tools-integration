...
### Coupon text in order summary
We add all discounts via `CustomLineItem`, which is visible for customers on invoice, so it's important to make it readable for them, to achive that we add support for i18n which do needed translations. To work it properly, you have to create folder (in `i18n` directory) for each language you use in a project (e.g `en`, `de`). Inside them create a file `test.json` in which put an object with text in that language (e.g. `{ "coupon": "Coupon code" }`). One more thing to do is to add `locale` property to a cart when it's created (it's most probably frontend thing).
